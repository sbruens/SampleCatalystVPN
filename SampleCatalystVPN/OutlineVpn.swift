...
public class OutlineVpn: NSObject {
  ...
  override private init() {
    ...
    getTunnelManager { manager in
      guard manager != nil else {
        return print("Tunnel manager not active. VPN not configured.")
      }
  ...
  public func start(
    _ tunnelId: String, configJson: [String: Any], _ completion: @escaping (Callback)
  ) {
  ...
  public func stop(_ tunnelId: String) {
    if !isActive(tunnelId) {
      return print("Cannot stop VPN, tunnel ID \(tunnelId)")
    }
    stopVpn()
  }
  ...
  private func startVpn(
    _ tunnelId: String?, configJson: [String: Any]?, isAutoConnect: Bool,
    _ completion: @escaping (Callback)
  ) {
    setupVpn { success in
      guard success else {
        print("Failed to setup VPN")
  ...
  private func restartVpn(
    _ tunnelId: String, configJson: [String: Any],
    completion: @escaping (Callback)
  ) {
    let message =
      [
        MessageKey.action: Action.restart, MessageKey.tunnelId: tunnelId,
  ...
  private func setupVpn(completion: @escaping (Bool) -> Void) {
    getOrCreateTunnelManager { manager in
      guard let manager else {
        print("Failed to setup tunnel manager")
  ...
  private func getOrCreateTunnelManager(
  ...
  private func setConnectVpnOnDemand(_ enabled: Bool) {
    ...
    self.tunnelManager?.saveToPreferences { error in
      if let error = error {
        return print("Failed to set VPN on demand to \(enabled): \(error)")
      }
    }
  }
  ...
  private func createTunnelManager(_ completion: @escaping ((NETunnelProviderManager?) -> Void)) {
    ...
    self.saveTunnelManager(manager) { success in
      guard success else {
        print("Failed to create new tunnel manager")
        return completion(nil)
      }
      print("Created new tunnel manager")
      return completion(manager)
    }
  }
  ...
  private func getTunnelManager(_ completion: @escaping ((NETunnelProviderManager?) -> Void)) {
    NETunnelProviderManager.loadAllFromPreferences { (managers, error) in
      guard error == nil, managers != nil else {
        completion(nil)
        return print("Failed to get tunnel manager: \(String(describing: error))")
      }

      print("Loaded \(managers!.count) tunnel managers")
  ...
  private func saveTunnelManager(
    _ manager: NETunnelProviderManager, _ completion: @escaping ((Bool) -> Void)
  ) {
    manager.saveToPreferences { error in
      ...
      self.tunnelManager?.loadFromPreferences { error in
        if let error = error {
          print("Failed to get tunnel manager: \(error)")
          return completion(false)
        }
        print("Saved VPN configuration")
        return completion(true)
      }
    }
  }
  ...
  private func retrieveActiveTunnelId() {
    ...
    self.sendVpnExtensionMessage([MessageKey.action: Action.getTunnelId]) { response in
      guard response != nil else {
        return print("Failed to retrieve the active tunnel ID")
      }
      guard let activeTunnelId = response?[MessageKey.tunnelId] as? String else {
        return print("Failed to retrieve the active tunnel ID")
      }
      print("Got active tunnel ID: \(activeTunnelId)")
  ...
  private func observeVpnStatusChange(_ manager: NETunnelProviderManager) {
    NotificationCenter.default.addObserver(
      self, selector: #selector(self.vpnStatusChanged),
   ...
   */
  private func sendVpnExtensionMessage(
    _ message: [String: Any],
    callback: @escaping (([String: Any]?) -> Void)
  ) {
    if tunnelManager == nil {
      return print("Cannot set an extension callback without a tunnel manager")
    }
    var data: Data
    do {
      data = try JSONSerialization.data(withJSONObject: message, options: [])
    } catch {
      return print("Failed to serialize message to VpnExtension as JSON")
    }
    let completionHandler: (Data?) -> Void = { data in
      ...
      do {
        ...
        {
          print("Received extension message: \(String(describing: response))")
          return callback(response)
        }
      } catch {
        print("Failed to deserialize the VpnExtension response")
      }
      callback(nil)
    }
    let session: NETunnelProviderSession = tunnelManager?.connection as! NETunnelProviderSession
    do {
      ...
      print("Failed to send message to VpnExtension")
    }
  }
